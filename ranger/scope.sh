...
handle_image() {
  ...
  echo "Extension: $extension" >> /tmp/ranger_debug.log
  case "$extension" in
  # Images classiques
  ...
  application/pdf)
    tmpimage="/tmp/ranger-pdf-preview-${RANDOM}.png"
    pdftoppm -f 1 -singlefile -png "${FILE_PATH}" "${tmpimage%.*}" >/dev/null
    if [ -f "$tmpimage" ]; then
      w3mimgdisplay "$tmpimage" >/dev/null 2>&1 && exit 6
    fi
    ;;
  esac
  # openscad_image() {
